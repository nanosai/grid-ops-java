...
public class TcpSocketTest {
    ...
    public void testRead_singleMessage_oneByteAtATime() throws IOException {
        ...
        MemoryBlockBatch msgDest      = new MemoryBlockBatch(1024);
        int           msgDstOffset = 0;

        tcpSocket.sourceLength = IapUtil.createMessage(tcpSocket.byteSource, 0);
        int msgCount = tcpSocket.readMessages(buffer, msgDest);
        ...
        msgCount = tcpSocket.readMessages(buffer, msgDest);
        assertEquals( 1, msgCount);
        assertEquals(16, tcpSocket.doSocketReadCallCount);
        assertNotNull(msgDest.blocks[0]);
        assertEquals (0, msgDest.blocks[0].startIndex);
        assertEquals (15, msgDest.blocks[0].endIndex);
